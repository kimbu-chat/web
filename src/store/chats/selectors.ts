...
export const getChatLastMessageIdSelector = (chatId: number) => (
  state: RootState,
): number | undefined => state.chats?.chats?.find((x: IChat) => x?.id === chatId)?.lastMessage?.id;
...
export const getSelectedChatInterlocutorNameSelector = (state: RootState) => {
  const selectedChat = state.chats?.chats?.find(
...
export const getChatByIdSelector = (chatId: number) => (state: RootState) =>
  state.chats.chats.find(({ id }) => id === chatId);
...
export const getChatsLoadingSelector = (state: RootState): boolean | undefined =>
  state.chats.loading;
