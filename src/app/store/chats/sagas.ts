...
export function* changeChatVisibilityStateSaga(action: ReturnType<typeof ChatActions.changeChatVisibilityState>): SagaIterator {
  const chat: Chat = action.payload;
...
export function* uploadAttachmentSaga(action: ReturnType<typeof ChatActions.uploadAttachmentRequestAction>): SagaIterator {
...
export function* changeSelectedChatSaga(action: ReturnType<typeof ChatActions.changeSelectedChat>): SagaIterator {
  if (action.payload !== -1 && !Number.isNaN(action.payload)) {
    ...
    if (!chatExists) {
      ...
      const { data, status } = httpRequest.call(yield call(() => httpRequest.generator({ chatId: action.payload })));
