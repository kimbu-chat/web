...
import { getMyFullNameSelector } from 'app/store/my-profile/selectors';
import { IChatsState, IMessageTypingApiRequest } from '../../models';
...
export class MessageTyping {
  ...
  static get saga() {
    return function* ({ payload }: ReturnType<typeof MessageTyping.action>): SagaIterator {
      ...
      const interlocutorName = yield select(getMyFullNameSelector);
